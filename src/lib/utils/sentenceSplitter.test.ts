import { splitSentences, splitFirstSentence } from "./sentenceSplitter";
...
describe("sentenceSplitter", () => {
  describe("splitFirstSentence", () => {
    ...
    it("should not split on periods inside parentheses", () => {
      const text =
        "The conflict (which started in Oct. 2023) continues. Peace talks have stalled.";
      const [first, rest] = splitFirstSentence(text);

      expect(first).toBe(
        "The conflict (which started in Oct. 2023) continues.",
      );
  ...
  describe("splitSentences", () => {
    ...
    it("should handle complex real-world example with abbreviations", () => {
      ...
      expect(sentences[2]).toBe(
        "Dr. Smith from the W.H.O. presented findings.",
      );
    });
    ...
    it("should handle multiple abbreviations in same sentence", () => {
      const text =
        "The U.S. and U.K. formed an alliance. The E.U. and U.N. responded quickly.";
      const sentences = splitSentences(text);
    ...
    it("should handle titles and honorifics", () => {
      const text =
        "Dr. Johnson met with Mr. Smith. Ms. Davis and Mrs. Wilson attended.";
      const sentences = splitSentences(text);
    ...
    it("should not split on periods in parentheses", () => {
      const text =
        "The conflict (which started in Oct. 2023) continues. Peace talks have begun.";
      const sentences = splitSentences(text);
