...
export class SearchExecutorService {
  ...
  async executeSearch(
    ...
    if (options.includeLocal && (query || filters.length > 0)) {
      results.localResults = this.searchLocal(
        query,
        filters,
        allCategoryStories,
        categories,
    ...
    if (options.includeHistorical && (query || filters.length > 0)) {
      const historicalData = await this.searchHistorical(
        query,
        filters,
        options,
      );
  ...
  private searchLocal(
    ...
    for (const [categoryId, stories] of Object.entries(allCategoryStories)) {
      ...
      if (
        categoryFilter &&
        categoryId.toLowerCase() !== categoryFilter.value.toLowerCase()
      ) {
        continue;
      }
  ...
  private async searchHistorical(
    ...
    try {
      ...
      // Add other params
      params.set("lang", dataLanguage.current);
      ...
      const results: SearchResult[] = data.results.map((result: any) => ({
        story: result.story as SearchStory,
      ...
      return {
        ...
        hasMore: data.hasMore || data.total > results.length,
      };
    } catch (error) {
