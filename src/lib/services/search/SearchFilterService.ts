...
import type { SearchFilter, FilterContext, FilterSuggestion } from "./types";
    ...
    if (typeMatch) {
      ...
      if (filterTypes.some((type) => type.startsWith(partial.toLowerCase()))) {
        return {
          ...
          suggestions: filterTypes.filter((type) =>
            type.startsWith(partial.toLowerCase()),
          ),
        };
      }
    }
